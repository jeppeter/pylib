...
def mul_handler(args,parser):
	...
	while k < len(args.subnargs):
		c = fileop.parse_int(args.subnargs[k])
		p = gmul(p,c)
		k = k + 1
...
def explist_handler(args,parser):
	loglib.set_logging(args)
	bnum = fileop.parse_int(args.subnargs[0])
	k = 1
	s = ''
	s += '[%02x'%(1)
...
def altable_handler(args,parser):
	loglib.set_logging(args)
	gnum = fileop.parse_int(args.subnargs[0])
...
def exp_handler(args,parser):
	loglib.set_logging(args)
	bnum = fileop.parse_int(args.subnargs[0])
	enum = fileop.parse_int(args.subnargs[1])
...
def format_uint8(arr,name,tab=0):
	s = ''
	s += format_tab_line(tab,'static uint8_t %s[] = {'%(name))
	idx = 0
	lastidx = 0
	curs = ''
	while idx < len(arr):
...
def format_uint32(arr,name,tab=0):
	s = ''
	s += format_tab_line(tab,'static uint32_t %s[] = {'%(name))
	idx = 0
	lastidx = 0
	curs = ''
	while idx < len(arr):
...
def formccode_handler(args,parser):
	loglib.set_logging(args)
	s = format_uint8(S,'s_box',0)
	s += format_tab_line(0,'')
	s += format_uint8(Si,'si_box',0)
	s += format_tab_line(0,'')
	s += format_uint8(rcon,'rcon_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T1,'t1_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T2,'t2_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T3,'t3_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T4,'t4_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T5,'t5_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T6,'t6_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T7,'t7_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(T8,'t8_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(U1,'u1_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(U2,'u2_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(U3,'u3_box',0)
	s += format_tab_line(0,'')
	s += format_uint32(U4,'u4_box',0)
	outfile = None
	if len(args.subnargs) > 0:
		outfile = args.subnargs[0]
	fileop.write_file(s,outfile)
	sys.exit(0)
	return
