...
class Node():
    ...
    def __repr__(self: T) -> str:
        ...
        if self._color == 0:
            cs = 'black'
        return "Key: " + str(self._key) + " Value: " + str(self.value) + " Color : " + cs
...
class RedBlackTree():
    ...
    def fix_insert(self: T, node: Node) -> None:
        logging.info('node %s'%(node.get_key()))
        while node.parent.is_red():
            if node.parent == node.parent.parent.right:
                u = node.parent.parent.left
                logging.info('parent.parent.left u %s'%(repr(u)))
                if u.is_red():
                    u.set_color("black")
                    node.parent.set_color("black")
                    logging.info('node parent %s'%())
                    node.parent.parent.set_color("red")
                    node = node.parent.parent
                else:
                    if node == node.parent.left:
                        logging.info('right_rotate before node %s'%(repr(node)))
                        node = node.parent
                        logging.info('right_rotate %s'%(node.get_key()))
                        self.right_rotate(node)
                    node.parent.set_color("black")
                    node.parent.parent.set_color("red")
                    logging.info('left_rotate node %s .parent.parent %s'%(repr(node),node.parent.parent.get_key()))
                    self.left_rotate(node.parent.parent)
            else:
                u = node.parent.parent.right
                logging.info('parent.parent.right u %s'%(u.get_key()))
                if u.is_red():
                    u.set_color("black")
                    node.parent.set_color("black")
                    node.parent.parent.set_color("red")
                    ...
                    if node == node.parent.right:
                        logging.info('left_rotate before %s'%(repr(node)))
                        node = node.parent
                        logging.info('left_rotate %s'%(repr(node)))
                        self.left_rotate(node)
                    logging.info('node %s'%(repr(node)))
                    node.parent.set_color("black")
                    node.parent.parent.set_color("red")
                    logging.info('right_rotate %s'%(node.parent.parent.get_key()))
    ...
    def insert(self: T, key: int) -> None:
        ...
        if y is not None:
            logging.info('parent %s'%(y.get_key()))
        node.parent = y
        if y is None:
            self.root = node
        elif node.get_key() < y.get_key():
            logging.info('left node %d'%(node.get_key()))
            y.left = node
        else:
            logging.info('right node %d'%(node.get_key()))
            y.right = node
...
def main():
    ...
    for v in sys.argv[1:]:
        iv = int(v)
        tree.insert(iv)
        sys.stdout.write('after insert %d\n'%(iv))
        tree.print_tree()
    for v in sys.argv[1:]:
        iv = int(v)
        tree.delete(iv)
        sys.stdout.write('after delete %d\n'%(iv))
        tree.print_tree()
    sys.exit(0)
