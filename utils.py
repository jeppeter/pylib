...
def format_header(f,args):
    bb = os.path.basename(f)
    hdrline = '%s'%(bb)
    hdrline = hdrline.upper()
    hdrline = re.sub('\\.','_',hdrline)
    result = os.urandom(16)
    rands = ''
    for b in result:
        rands += '%02x'%(b)
    rands = rands.upper()
    hdrline = '__%s_%s__'%(hdrline,rands)
    outs = format_tab_line(0,'// this file is generated by program')
    outs += format_tab_line(0,'#ifndef %s'%(hdrline))
    outs += format_tab_line(0,'#define %s'%(hdrline))
    outs += format_tab_line(0,'')
    if args.cppmode:
        outs += format_tab_line(0,'#ifdef __cplusplus')
        outs += format_tab_line(0,'extern "C" {')
        outs += format_tab_line(0,'#endif /* __cplusplus*/')
        outs += format_tab_line(0,'')
        outs += format_tab_line(0,'#ifdef __cplusplus')
        outs += format_tab_line(0,'};')
        outs += format_tab_line(0,'#endif /* __cplusplus*/')
    ...
    outs += format_tab_line(0,'')
    outs += format_tab_line(0,'#endif /* %s */'%(hdrline))
    write_file(outs,f)
    return
